...
export const CustomerDashboard: React.FC<CustomerDashboardProps> = ({ user, onLogout }) => {
  ...
  const [checkoutData, setCheckoutData] = useState({
    ...
    cityName: '',
    courier: '',
    shippingMethod: '', // Holds the selected service name (e.g., "JNE REG")
    shippingCost: 0,
    paymentMethod: '', 
    paymentProofUrl: '' 
  });
  ...
  const handleCheckShipping = () => {
    ...
    // SIMULATION (Based on index.html analysis)
    // In a real app, this would be a POST to your backend which calls BinderByte/RajaOngkir Cost API
    // CORS usually blocks direct frontend calls to these APIs.
    setTimeout(() => {
      const mockResults = [
  ...
  const handlePaymentProofUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      try {
        ...
        showToast('error', 'Error', "Failed to upload payment proof.");
      } finally {
    ...
    try {
        ...
        if (messages.length > 0) {
            ...
            setSettingsForm(prev => ({ ...prev, newPassword: '', confirmPassword: '' }));
             ...
             showToast('success', 'Sukses', "Settings saved.");
        }
  ...
  const startChatWithMerchant = (merchant: User, initialMessage?: string, attachmentUrl?: string) => {
    if (!merchant.id) {
        showToast('error', 'Error', "Merchant ID is invalid.");
        return;
    }
    ...
    if (!exists) {
        setActiveConversations(prev => [...prev, {
            ...
            lastMessage: initialMessage || 'Start conversation',
            timestamp: new Date(),
            unreadCount: 0
        }]);
    }
  ...
  const handleDiscussCatalogItem = (merchant: User, item: CatalogItem) => {
    ...
    startChatWithMerchant(merchant, message, item.imageUrl);
  };
      ...
      try {
          ...
          if (orderToReceive.merchantId) {
            await createTransaction({
                ...
                description: `Payment for order ${orderToReceive.designName}`,
                status: 'Success',
                type: 'Payment',
                date: new Date().toISOString()
            });
    ...
    <div className="h-screen bg-[#FDFBF7] flex relative overflow-hidden">
      ...
      {isCancelModalOpen && orderToCancel && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-md p-4 animate-fade-in">
              <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md overflow-hidden border border-white animate-scale-in">
                  <div className="p-6 border-b border-stone-100 bg-red-50 flex justify-between items-center">
                      <h3 className="text-xl font-serif font-bold text-red-700 flex items-center gap-2">
                          <Trash2 size={24} /> Batalkan Pesanan
                      </h3>
                      <button onClick={() => setCancelModalOpen(false)} className="p-2 hover:bg-stone-100 rounded-full text-stone-400 hover:text-red-600 transition-colors">
                          <X size={20} />
                      </button>
                  </div>
                  <div className="p-8 space-y-6">
                      ...
                      <div>
                          <label className="block text-xs font-bold text-stone-500 uppercase tracking-wider mb-2">Alasan Pembatalan</label>
                          <textarea 
                              className="w-full p-4 border border-stone-200 rounded-xl bg-stone-50 focus:outline-none focus:ring-2 focus:ring-red-200 h-32"
                              placeholder="Contoh: Berubah pikiran, salah ukuran..."
                              value={cancelReason}
                      ...
                      <div className="flex gap-4 pt-2">
                          ...
                          <button 
                              ...
                              className="flex-1 py-3 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 shadow-lg shadow-red-100 flex items-center justify-center gap-2 disabled:opacity-70"
                          >
                              {isProcessingOrder ? <Loader2 className="animate-spin" size={18} /> : "Ya, Batalkan"}
                          </button>
                      </div>
                  </div>
              </div>
      ...
      {/* ... Complaint & Receive Modals ... */}
      {isComplaintModalOpen && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-md p-4 animate-fade-in">
              <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden border border-white">
                  <div className="p-6 border-b border-stone-100 bg-red-50 flex justify-between items-center"><h3 className="text-xl font-serif font-bold text-red-700 flex items-center gap-2"><AlertTriangle size={24} /> Ajukan Komplain</h3><button onClick={() => setComplaintModalOpen(false)} className="p-2 hover:bg-stone-100 rounded-full text-stone-400 hover:text-red-600 transition-colors"><X size={20} /></button></div>
                  <div className="p-8 space-y-6">
                      <p className="text-sm text-stone-500">Mohon jelaskan masalah yang Anda alami dengan pesanan ini. Sertakan bukti foto jika ada.</p>
                      <div><label className="block text-xs font-bold text-stone-500 uppercase tracking-wider mb-2">Alasan Komplain</label><textarea className="w-full p-4 border border-stone-200 rounded-xl bg-stone-50 focus:outline-none focus:ring-2 focus:ring-red-200 h-32" placeholder="Contoh: Jahitan tidak rapi, ukuran tidak sesuai..." value={complaintReason} onChange={(e) => setComplaintReason(e.target.value)} /></div>
                      <div><label className="block text-xs font-bold text-stone-500 uppercase tracking-wider mb-2">Bukti Foto (Opsional)</label><div className="flex gap-4 items-center"><label className={`flex-1 cursor-pointer ${isUploadingComplaint ? 'opacity-50 pointer-events-none' : ''}`}><div className="h-24 border-2 border-dashed border-stone-300 rounded-xl bg-stone-50 hover:bg-white transition-colors flex flex-col items-center justify-center text-center">{isUploadingComplaint ? <Loader2 className="animate-spin text-stone-400" size={20} /> : <UploadCloud className="text-stone-400 mb-1" size={20} />}<span className="text-xs font-medium text-stone-500">{isUploadingComplaint ? 'Uploading...' : 'Upload Foto'}</span><input type="file" className="hidden" accept="image/*" onChange={handleComplaintImageUpload} /></div></label>{complaintImage && (<div className="w-24 h-24 rounded-xl overflow-hidden bg-stone-100 border border-stone-200"><img src={complaintImage} alt="Bukti" className="w-full h-full object-cover" /></div>)}</div></div>
                      <div className="flex gap-4 pt-2"><button onClick={() => setComplaintModalOpen(false)} className="flex-1 py-3 border border-stone-200 text-stone-600 rounded-xl font-bold hover:bg-stone-50">Batal</button><button onClick={handleSubmitComplaint} disabled={isSubmittingComplaint || isUploadingComplaint} className="flex-1 py-3 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 shadow-lg shadow-red-100 flex items-center justify-center gap-2 disabled:opacity-70">{isSubmittingComplaint ? <Loader2 className="animate-spin" size={18} /> : <Send size={18} />} Kirim Komplain</button></div>
                  </div>
              </div>
          </div>
      )}
      {isReceiveConfirmOpen && orderToReceive && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-md p-4 animate-fade-in">
              <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md overflow-hidden border border-white animate-scale-in">
                  <div className="p-8 text-center space-y-6"><div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto text-green-600 shadow-sm"><CheckCircle size={40} /></div><div><h3 className="text-2xl font-serif font-bold text-berry-rich mb-2">Terima Pesanan?</h3><p className="text-stone-500">Apakah Anda yakin pesanan <strong>{orderToReceive.designName}</strong> sudah diterima dengan baik?</p><p className="text-xs text-stone-400 mt-2">Tindakan ini akan melepaskan dana ke Mitra dan menyelesaikan pesanan.</p></div><div className="flex gap-3 pt-4"><button onClick={() => setReceiveConfirmOpen(false)} className="flex-1 py-3 border border-stone-200 text-stone-600 rounded-xl font-bold hover:bg-stone-50 transition-colors">Batal</button><button onClick={confirmOrderReceived} disabled={isProcessingOrder} className="flex-1 py-3 bg-green-600 text-white rounded-xl font-bold hover:bg-green-700 shadow-lg shadow-green-100 flex items-center justify-center gap-2 disabled:opacity-70">{isProcessingOrder ? <Loader2 className="animate-spin" size={20} /> : "Ya, Terima"}</button></div></div>
              </div>
          </div>
      )}

      {/* Customer Order Details Modal (with details rendering) ... */}
      {selectedOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-berry-rich/20 backdrop-blur-md p-4 animate-fade-in">
          ...
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-2xl overflow-hidden max-h-[90vh] flex flex-col border border-white animate-fade-in-up">
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-8">
              ...
              {checkoutStep === 3 && (
                <div className="space-y-8 animate-fade-in">
                  <div>
                    <label className="block font-bold mb-3 text-berry-rich flex items-center gap-2"><MapPin size={18} className="text-brand-gold" /> {t.shippingAddress}</label>
                    <textarea className="w-full p-4 border border-stone-200 rounded-2xl focus:ring-2 focus:ring-brand-gold/20 focus:border-brand-gold outline-none h-24 bg-white shadow-sm mb-4" placeholder="Jalan Mawar No. 123..." value={checkoutData.address} onChange={(e) => setCheckoutData({...checkoutData, address: e.target.value})} />
                    
                    {/* Region Selection */}
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div>
                        <label className="block text-xs font-bold text-stone-500 uppercase mb-2">Provinsi</label>
                        <select className="w-full p-3 bg-white border border-stone-200 rounded-xl text-sm" value={checkoutData.provinceId} onChange={handleProvinceChange}>
                          <option value="">Pilih Provinsi</option>
                          {provinces.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                        </select>
                      </div>
                      <div>
                        <label className="block text-xs font-bold text-stone-500 uppercase mb-2">Kota/Kabupaten {isLoadingRegions && <Loader2 className="inline animate-spin ml-1" size={12} />}</label>
                        <select className="w-full p-3 bg-white border border-stone-200 rounded-xl text-sm" value={checkoutData.cityId} onChange={handleCityChange} disabled={!checkoutData.provinceId}>
                  ...
                  <div>
                    <label className="block font-bold mb-3 text-berry-rich flex items-center gap-2"><Truck size={18} className="text-brand-gold" /> {t.courier}</label>
                    <div className="flex gap-2 mb-4">
                        <select className="w-full p-4 bg-white border border-stone-200 rounded-2xl focus:ring-2 focus:ring-brand-gold/20 outline-none shadow-sm appearance-none" value={checkoutData.courier} onChange={(e) => setCheckoutData({...checkoutData, courier: e.target.value})}>
                            <option value="">Pilih Kurir...</option>
                            <option value="jne">JNE</option>
                            <option value="pos">POS Indonesia</option>
                            <option value="tiki">TIKI</option>
                        </select>
                        <button onClick={handleCheckShipping} disabled={isLoadingShippingCost} className="px-6 py-3 bg-stone-800 text-white rounded-xl font-bold hover:bg-black transition-colors whitespace-nowrap">
                            {isLoadingShippingCost ? <Loader2 className="animate-spin" /> : "Cek Ongkir"}
                        </button>
                    </div>

                    {/* Shipping Results */}
                    {shippingOptions.length > 0 && (
                        <div className="space-y-3 bg-stone-50 p-4 rounded-2xl border border-stone-200 mb-6">
                            <p className="text-xs font-bold text-stone-500 uppercase">Pilih Layanan</p>
                            {shippingOptions.map((opt, idx) => (
                                <label key={idx} className={`flex items-center justify-between p-3 border rounded-xl cursor-pointer transition-all ${checkoutData.shippingMethod === opt.service ? 'border-brand-gold bg-white shadow-sm' : 'border-stone-200 hover:bg-white'}`}>
                                    <div className="flex items-center gap-3">
                                        <input 
                                            type="radio" 
                                            name="shipping" 
                                            className="w-4 h-4 text-brand-gold focus:ring-brand-gold"
                                            checked={checkoutData.shippingMethod === opt.service}
                                            onChange={() => setCheckoutData({...checkoutData, shippingMethod: opt.service, shippingCost: opt.cost})}
                                        />
                                        <div>
                                            <p className="font-bold text-sm text-stone-800">{opt.service} <span className="font-normal text-stone-500">({opt.description})</span></p>
                                            <p className="text-xs text-stone-400">Estimasi: {opt.etd}</p>
                                        </div>
                                    </div>
                                    <p className="font-serif font-bold text-berry-rich">Rp {opt.cost.toLocaleString()}</p>
                                </label>
                            ))}
                        </div>
                    )}
                  ...
                  <div>
                    <label className="block font-bold mb-3 text-berry-rich flex items-center gap-2">
                        <CardIcon size={18} className="text-brand-gold" /> {t.paymentMethod}
                    </label>
                    <div className="grid grid-cols-2 gap-3 mb-6">
                        {(selectedMerchant.bankDetails?.bankName || selectedMerchant.bankAccount) && (<button onClick={() => setCheckoutData({...checkoutData, paymentMethod: 'Bank Transfer'})} className={`p-4 rounded-2xl text-sm font-bold transition-all border ${checkoutData.paymentMethod === 'Bank Transfer' ? 'border-berry-rich bg-berry-rich text-white shadow-lg' : 'border-stone-200 bg-white text-stone-600 hover:border-berry-rich/30'}`}>{t.bankTransfer}</button>)}
                        {selectedMerchant.ewalletDetails?.walletName && (<button onClick={() => setCheckoutData({...checkoutData, paymentMethod: 'E-Wallet'})} className={`p-4 rounded-2xl text-sm font-bold transition-all border ${checkoutData.paymentMethod === 'E-Wallet' ? 'border-berry-rich bg-berry-rich text-white shadow-lg' : 'border-stone-200 bg-white text-stone-600 hover:border-berry-rich/30'}`}>{t.eWallet}</button>)}
                    </div>

                    {/* IMPROVED COPY BUTTONS HERE */}
                    {checkoutData.paymentMethod === 'Bank Transfer' && (
                        <div className="bg-stone-50 border border-stone-200 rounded-2xl p-6 space-y-4 animate-fade-in">
                            <div className="bg-white p-5 rounded-2xl border border-stone-100 shadow-sm relative group hover:border-brand-gold/50 transition-colors">
                                <p className="text-xs font-bold text-stone-400 uppercase tracking-wider mb-2">Transfer Destination</p>
                                {selectedMerchant.bankDetails ? (
                                    <>
                                        <div className="flex items-center justify-between mb-1">
                                            <p className="text-2xl font-mono font-bold text-berry-rich tracking-tight">{selectedMerchant.bankDetails.accountNumber}</p>
                                            <button 
                                                onClick={() => handleCopy(selectedMerchant.bankDetails!.accountNumber)} 
                                                className="p-2 bg-stone-100 hover:bg-green-50 text-stone-500 hover:text-green-600 rounded-xl transition-all shadow-sm active:scale-95 flex items-center gap-2 font-bold text-xs"
                                            >
                                                {copiedText === selectedMerchant.bankDetails.accountNumber ? <><Check size={16} /> Salin</> : <><Copy size={16} /> Salin</>}
                                            </button>
                                        </div>
                                        <p className="font-bold text-stone-800 text-lg">{selectedMerchant.bankDetails.bankName.toUpperCase()}</p>
                                        <p className="text-sm text-stone-500 font-medium">a.n. {selectedMerchant.bankDetails.accountHolder}</p>
                                    </>
                                ) : (
                                    <p className="text-lg font-mono font-bold text-berry-rich">{selectedMerchant.bankAccount || "Ask Merchant via Chat"}</p>
                                )}
                            </div>
                            <div><label className="block text-sm font-bold text-stone-600 mb-2">Upload Payment Proof (Resi)</label><div className="flex gap-4 items-center"><label className={`flex-1 cursor-pointer ${isUploadingProof ? 'opacity-50 pointer-events-none' : ''}`}><div className="h-24 border-2 border-dashed border-stone-300 rounded-xl bg-white hover:bg-stone-50 transition-colors flex flex-col items-center justify-center text-center p-2">{isUploadingProof ? (<Loader2 className="animate-spin text-berry-rich" size={20} />) : (<UploadCloud className="text-stone-400 mb-1" size={20} />)}<span className="text-xs font-medium text-stone-500">{isUploadingProof ? 'Uploading...' : 'Click to upload proof'}</span><input type="file" className="hidden" accept="image/*" onChange={handlePaymentProofUpload} /></div></label>{checkoutData.paymentProofUrl && (<div className="h-24 w-24 rounded-xl overflow-hidden border border-stone-200 relative"><img src={checkoutData.paymentProofUrl} alt="Proof" className="w-full h-full object-cover" /><div className="absolute inset-0 bg-green-500/20 flex items-center justify-center"><CheckCircle size={20} className="text-white drop-shadow-md" /></div></div>)}</div></div>
                        </div>
                    )}

                    {checkoutData.paymentMethod === 'E-Wallet' && selectedMerchant.ewalletDetails && (
                        <div className="bg-stone-50 border border-stone-200 rounded-2xl p-6 space-y-4 animate-fade-in">
                            <div className="bg-white p-5 rounded-2xl border border-stone-100 shadow-sm relative group hover:border-brand-gold/50 transition-colors">
                                <p className="text-xs font-bold text-stone-400 uppercase tracking-wider mb-2">{selectedMerchant.ewalletDetails.walletName} Destination</p>
                                <div className="flex items-center justify-between mb-1">
                                    <p className="text-2xl font-mono font-bold text-berry-rich tracking-tight">{selectedMerchant.ewalletDetails.phoneNumber}</p>
                                    <button 
                                        onClick={() => handleCopy(selectedMerchant.ewalletDetails!.phoneNumber)} 
                                        className="p-2 bg-stone-100 hover:bg-green-50 text-stone-500 hover:text-green-600 rounded-xl transition-all shadow-sm active:scale-95 flex items-center gap-2 font-bold text-xs"
                                    >
                                        {copiedText === selectedMerchant.ewalletDetails.phoneNumber ? <><Check size={16} /> Salin</> : <><Copy size={16} /> Salin</>}
                                    </button>
                                </div>
                            </div>
                            <div><label className="block text-sm font-bold text-stone-600 mb-2">Upload Payment Proof</label><div className="flex gap-4 items-center"><label className={`flex-1 cursor-pointer ${isUploadingProof ? 'opacity-50 pointer-events-none' : ''}`}><div className="h-24 border-2 border-dashed border-stone-300 rounded-xl bg-white hover:bg-stone-50 transition-colors flex flex-col items-center justify-center text-center p-2">{isUploadingProof ? (<Loader2 className="animate-spin text-berry-rich" size={20} />) : (<UploadCloud className="text-stone-400 mb-1" size={20} />)}<span className="text-xs font-medium text-stone-500">{isUploadingProof ? 'Uploading...' : 'Click to upload proof'}</span><input type="file" className="hidden" accept="image/*" onChange={handlePaymentProofUpload} /></div></label>{checkoutData.paymentProofUrl && (<div className="h-24 w-24 rounded-xl overflow-hidden border border-stone-200 relative"><img src={checkoutData.paymentProofUrl} alt="Proof" className="w-full h-full object-cover" /><div className="absolute inset-0 bg-green-500/20 flex items-center justify-center"><CheckCircle size={20} className="text-white drop-shadow-md" /></div></div>)}</div></div>
                        </div>
                    )}
                  </div>
                </div>
              ...
              {checkoutStep === 4 && (
                <div className="space-y-8 animate-fade-in">
                  <div className="text-center"><div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4 text-green-600 animate-pulse"><FileText size={40} /></div><h4 className="text-3xl font-serif font-bold text-berry-rich">{t.orderSummary}</h4></div><div className="bg-stone-50 p-8 rounded-[2rem] space-y-4 border border-stone-100 relative overflow-hidden"><div className="absolute top-0 right-0 w-32 h-32 bg-brand-gold/5 rounded-full -translate-y-1/2 translate-x-1/2"></div><div className="flex justify-between pb-4 border-b border-stone-200 relative z-10"><span className="text-stone-500 font-medium">Design</span><span className="font-bold text-berry-rich font-serif text-lg">{checkoutItem.title}</span></div><div className="flex justify-between pb-4 border-b border-stone-200 relative z-10"><span className="text-stone-500 font-medium">Merchant</span><span className="font-bold text-stone-800">{selectedMerchant.brandName || selectedMerchant.name}</span></div><div className="flex justify-between pb-4 border-b border-stone-200 relative z-10"><span className="text-stone-500 font-medium">Shipping</span><span className="font-bold text-stone-800">{checkoutData.courier.toUpperCase()} - {checkoutData.shippingMethod}</span></div><div className="flex justify-between pt-2 relative z-10"><span className="text-lg font-bold text-berry-rich">{t.totalPrice}</span><span className="text-2xl font-serif font-bold text-brand-gold">Rp {((checkoutItem.price || 0) + checkoutData.shippingCost).toLocaleString()}</span></div>{checkoutData.paymentProofUrl && (<div className="pt-4 border-t border-stone-200 relative z-10"><p className="text-xs font-bold text-stone-500 mb-2">Attached Payment Proof:</p><img src={checkoutData.paymentProofUrl} alt="Proof" className="h-16 rounded-lg border border-stone-200" /></div>)}<p className="text-xs text-stone-400 mt-2 text-center italic relative z-10">{t.finalPriceNote}</p></div>
                </div>
              )}
            </div>

            <div className="p-6 border-t border-stone-100 bg-white flex justify-between gap-4 rounded-b-[2rem]">
              {checkoutStep > 1 && (<button onClick={() => setCheckoutStep(prev => prev - 1)} className="px-8 py-4 border border-stone-200 text-stone-600 rounded-xl font-bold hover:bg-stone-50 transition-colors">{t.back}</button>)}
              {checkoutStep < 4 ? (<button onClick={() => setCheckoutStep(prev => prev + 1)} className="px-8 py-4 bg-berry-rich text-white rounded-xl font-bold hover:bg-berry-dark transition-all flex-1 flex items-center justify-center gap-3 shadow-lg hover:shadow-berry-rich/30">{t.next} <ChevronRight size={20} /></button>) : (<button onClick={handleSubmitOrder} className="px-8 py-4 bg-gradient-to-r from-green-600 to-green-700 text-white rounded-xl font-bold hover:shadow-xl transition-all flex-1 flex items-center justify-center gap-3 shadow-green-200"><CheckCircle size={20} /> {t.submitOrder}</button>)}
            </div>
          </div>
        </div>
      )}
        ...
        <div className="p-6 md:p-12 max-w-6xl mx-auto">
                      ...
                      <div className="grid md:grid-cols-3 gap-8">
                        {merchants.map(merchant => (
                          <div key={merchant.id} className="bg-white rounded-[2rem] shadow-sm border border-stone-100 overflow-hidden hover:shadow-2xl transition-all duration-300 group hover:-translate-y-2 relative">
                            ...
                            <div className="p-8">
                                ...
                                <p className="text-sm font-medium text-stone-600 mb-6 line-clamp-2">{merchant.bio || "Professional Tailoring Service"}</p>
                        ...
                        {merchants.length === 0 && (
                            <div className="col-span-3 text-center py-10 bg-white rounded-[2rem] border border-stone-100">
                                <p className="text-stone-400 italic mb-2">No merchants available yet.</p>
                                <button onClick={() => { setIsLoadingMerchants(true); getAllMerchants().then(setMerchants).finally(() => setIsLoadingMerchants(false)); }} className="text-berry-rich text-sm font-bold flex items-center justify-center gap-2 mx-auto"><RefreshCw size={14} /> Refresh List</button>
                            </div>
                        )}
                      </div>
                  )}
                ...
                <div className="bg-white rounded-[3rem] p-10 border border-stone-100 shadow-xl animate-fade-in">
                   ...
                   <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                         {(selectedMerchant as any).catalog && (selectedMerchant as any).catalog.length > 0 ? ((selectedMerchant as any).catalog.map((item: CatalogItem) => (<div key={item.id} className="rounded-[2rem] overflow-hidden shadow-sm border border-stone-100 flex flex-col h-full bg-white hover:shadow-xl transition-shadow duration-300 group"><div className="overflow-hidden h-64"><img src={item.imageUrl} alt={item.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" /></div><div className="p-6 flex flex-col flex-1"><div className="flex justify-between items-start mb-3"><p className="font-serif font-bold text-berry-rich text-xl">{item.title}</p><span className="text-brand-gold font-bold text-sm bg-brand-gold/5 px-2 py-1 rounded-lg">{item.price ? `Rp ${item.price.toLocaleString()}` : ''}</span></div><p className="text-sm text-stone-500 mb-6 flex-1 leading-relaxed">{item.description}</p><div className="flex gap-3 mt-auto"><button onClick={() => handleDiscussCatalogItem(selectedMerchant, item)} className="flex-1 py-3 bg-stone-50 text-stone-600 rounded-xl hover:bg-stone-100 transition-colors font-bold flex items-center justify-center gap-2 text-xs uppercase tracking-wide"><MessageSquare size={16} /> Chat</button><button onClick={() => openCheckout(selectedMerchant, item)} className="flex-1 py-3 bg-berry-rich text-white rounded-xl hover:bg-berry-dark transition-colors font-bold flex items-center justify-center gap-2 text-xs uppercase tracking-wide shadow-md"><ShoppingBag size={16} /> {t.preOrder}</button></div></div></div>))) : (<p className="text-stone-400 col-span-3 text-center py-10 font-light italic">Merchant has not uploaded any projects yet.</p>)}
                      </div>
                   </div>
                </div>
              )}
          ...
          {activeTab === 'orders' && (
            <div className="space-y-8 animate-fade-in-up">
              ...
              {myOrders.length === 0 ? (<div className="text-center py-20 bg-white rounded-[2rem] border border-stone-100"><ShoppingBag size={48} className="mx-auto text-stone-300 mb-4" /><p className="text-stone-500">{t.myOrders} is empty. Start finding a partner!</p></div>) : (
                <div className="grid gap-6">
                    ...
                    <div key={order.id} className="bg-white p-6 rounded-[2rem] shadow-sm border border-stone-100 flex flex-col md:flex-row gap-8 items-center hover:shadow-xl transition-all duration-300 hover:border-brand-gold/30 group cursor-pointer" onClick={() => setSelectedOrder(order)}>
                        ...
                        <div className="flex justify-between items-start mb-3"><div><h3 className="text-2xl font-serif font-bold text-berry-rich">{order.designName}</h3><p className="text-stone-500 font-medium mt-1">by {order.merchantName}</p></div><span className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wide border ${order.status === OrderStatus.PRODUCTION ? 'bg-yellow-50 text-yellow-700 border-yellow-200' : (order.status === OrderStatus.COMPLETED ? 'bg-green-50 text-green-700 border-green-200' : (order.status === OrderStatus.COMPLAINT ? 'bg-red-50 text-red-700 border-red-200' : (order.status === 'Cancelled' ? 'bg-red-50 text-red-700 border-red-200' : 'bg-blue-50 text-blue-700 border-blue-200')))}`}>{order.status}</span></div>
                        <div className="w-full bg-stone-100 h-2.5 rounded-full mt-6 overflow-hidden"><div className={`h-full rounded-full shadow-[0_0_10px_#9a2768] ${order.status === 'Cancelled' ? 'bg-stone-300' : 'bg-gradient-to-r from-berry-rich to-brand-gold'}`} style={{ width: order.status === OrderStatus.PRODUCTION ? '60%' : (order.status === OrderStatus.COMPLETED ? '100%' : (order.status === 'Cancelled' ? '100%' : '20%')) }} /></div>
                        <div className="flex justify-between items-center mt-3">
                            <div className="flex gap-4 text-[10px] uppercase tracking-widest text-stone-400 font-bold"><span>{t.statusConsultation}</span><span>{t.statusDesign}</span><span>{t.statusProduction}</span><span>{t.statusDelivery}</span></div>
